...
In \ref{sec:curated} we identified four main axis of improvements in the field of Fog computing:
\begin{enumerate}
	\item Lack of \gls{SLA}
	\item Smarter auction-based placement mechanisms
	\item FaaS-specific mechanisms
	\item Security and privacy
\end{enumerate}}. One is still missing from this list: scientific comparison. The field currently lacks the ability to compare algorithms, placement decisions and so on. Crucial is also the ability to adapt the \gls{FaaS} paradigm as we test algorithms in order to identify and measure the room for improvement and the challenges. As we saw earlier the field does not lack ideas to place functions but lacks ways to test them on real problems rather than simulations.
