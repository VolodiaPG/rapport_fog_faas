...
\addbibresource{biblio-internship.bib} %added
...
\gls{FaaS}-enabled Fog would enable an applied real world usage of the Fog. However, as depicted in \ref{sec:curated} we identified four main axes of necessary improvements to go this direction:
\begin{enumerate}
	\item introduce \glspl{SLA} that enables the users to set their expectations and have them respected
	\item smarter auction-based placement mechanisms that enable full-scale incentives to both join and compete on the Fog
	\item \gls{FaaS}-specific mechanisms adapted to the Fog
	\item security and privacy
\end{enumerate}.
As this is a research work, we expect “scientific comparison” to be listed

Indeed the community presents promising placement solutions but fails to both compare them against each other and to test them on real scenarios instead of relying extensively on simulations. Thus it is not clear what to expect from the Fog as of today. This is the first reason why we choose to participate to the community effort producing a framework able to be experimented upon instead of simulated. A second key point is to be able to face implementation issues that would – through simulation – not be observed and solved.

In the next subsections we examine our proposed placement mechanism. We will detail the work done on our own open framework. Finally, we will detail how we plan to evaluate this solution and platform against state-of-the-art solutions.

\todo{Explain the development of existing functions as well as looking for existing framework + application list (usually not Fog friendly)}

\subsection{Placing functions in The Fog}

This internship focuses on the implementation of a new idea for placing functions in the Fog using economics as the main layer of dialogue between actors, much like what is already in place in the Cloud. After introducing our view of the Fog, we will detail our placement behaviors, how and why we chose to implement our own framework and the evaluation contributions regarding this work.

\subsubsection{Context and challenges}

The Fog is a distributed network made of fog nodes – computers – that work together to bring latency and bandwidth benefits to the Cloud customer. In time, Fog will be so entangled with the Cloud that clients should not be able to make the difference between executing their applications on the edge or on the Cloud. At least when they do not have any explicit requirements making them Fog-dependent, latency would be one. This means pricing should be indistinguishable from the current offerings. This also means logistics should be on point: Fog nodes are present everywhere not to degrade the usage of any applications explicitly relying on them.

This indicates a need to avoid the “vendor lock-in” nightmare, where multiple resource providers are on a market, but as each one proposes the same kind of service, each one still manages to do it in a proprietary manner, making a multiple-provider scenario impossible. This is what we are observing currently in the Cloud, with global companies trying to rely on multiple providers to avoid downtime, security issues, and to benefit from the best of each platform. Such as problem in Fog would be highly inefficient. It would only mean two things: either all providers are present at the same location, thus putting a lot of Fog nodes at the same place for no further benefits than to compete; or it would mean that geographical zones would be exclusive of a handful providers. In each case, it would be a nightmare for the client to deploy an application, let's say at an European-level for example.

In our work, we consider fog nodes are owned by different actors. This way, both private (enterprises) and public (cities) can provide computational resources to the community, while expecting sufficient gains to be able to sustain this sharing, or to make benefits. The more fog nodes join the network, the more competition and the more affordable it is for clients willing to execute applications gaining. We consider that any provider can be in competition, but using a standard platform to do so, so that it doesn't hurt the client nor makes the technology useless.

Considering this ownership of fog nodes, as introduced earlier, a good mechanism to introduce competition in a controlled, independent and objective manner are auctions. \improvement{Add info about benefits and properties of auctions}

\subsubsection{Placing functions}

Placing functions is a challenge. Adding that Fog nodes are independent actors that choose to join the network adds a difficulty layer. Especially since Fog nodes are able to specify their prices and independently choose their behaviors. Auctions help to make sure competition is maintained and as such prices are not artificially set that would harm the final client.

Auctions also help translating what metrics are considered important by a fog node in a single number: the price – that being virtual or real. As an example, Fog nodes near the \gls{IoT} sources – at the edge of the network – could have more concerns about energy usage than fog nodes running off a data-center, and as such make high energy consuming clients pay more.

With money in the equation, Fog nodes will not trust each other, as such we need to introduce the first component of our architecture: the \emph{marketplace}. It is the only software that is trusted by all Fog nodes.
...
The marketplace is the component of the framework which is accessible to the public. It can be though as a middle-ware acting as a high-level gateway to the consumer that wants to deploy and manage his functions. 

As the marketplace offers to only manage functions, clients may need a higher-level solution to manage their applications, introducing the “Deployment Manager”. This piece of software would – in the future – be responsible of managing the deployment of the client's application on the Fog. It would be aware of what \gls{IoT} devices are expected as it would know which one are currently connected to the Fog. Its role would then to ask the Marketplace to provision functions as it would be deemed necessary from his programming. The idea is to externalize the application managing from the Fog network and function execution management. That way, in case of failure or inability to provision part of the application, the task would be handled by a policy specified by the client. This would especially be relevant in case of a critical application that would need to be aware it simply cannot be executed and has to transmit the information to a human, for example. Or in a more simple fashion, needs to consider changing its requirements to be more adequate.
...
\begin{figure}[H]
	...
	\caption{step 0 — \gls{IoT} devices just connected\\		
...
\begin{figure}[H]
	...
	\caption{step 1 — \gls{SLA} creation and distribution\\		
...
\begin{figure}[H]
	...
	\caption{step 2 — Bidding\\		
		\textit{Upon receiving the \gls{SLA}, each Fog node bids on it and return their price to the marketplace. This represents what they think the function will cost to be executed.}}
	\label{fig:our_placement2}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{./assets/OurPlacementOriginal-Page-4.png}
	\caption{step 3 — Payment and provisioning\\
...
\begin{description}
	\item[Step 0 (\cref{fig:our_placement0})]{simply answers to the question: “how do the application is made aware the necessary \gls{IoT} devices are connected to run properly”. 
	The answer is through a simple “ping” informing the Deployment Manager that part of its hardware is connected and it may need to take a decision to integrate it in the deployed application.
	}
	\item[Step 1 (\cref{fig:our_placement1})] {is about the creation of and \gls{SLA}. When the Deployment Manager wants to provision a new function on the Fog network, it will be his responsibility to split his applications and constitute functions that will be executed on the Fog. Those functions are then described by a \gls{SLA} that wraps multiple points such as CPU, RAM, latency.
	...
	\item[Step 2 (\cref{fig:our_placement2})] {is the bidding process. Every fog node takes a look at the \gls{SLA} and bids. Thanks to an appropriate \improvement{Ref the appropriate auction mecha} auctioning mechanisms, it will represent the actual value – the truthful value – of the function through the eyes of the Fog node. Once this process has been done, the bid is returned back to the Marketplace.}
...
\begin{itemize}
	...
	\item not abandoned
	\item extensively and easily modifiable (network topology, distribution process for the functions)
	\item ready to be deployed on a real testbed
	\item easily measurable
	\item uses standard technologies so that we can benefit from already developed application samples and from the latest state-of-the-art methods to optimize \gls{FaaS}
\end{itemize}.

One key aspect we kept in mind was the time and efforts necessary to understand and re-purpose a framework to fit our needs versus the time we would need to develop our own solution.

In the community, two frameworks caught our eyes:
\todo{The comparison of frameworks}
\todo{Add more details about fogbus 2}
\begin{description}
	\item[\cite{deng_fogbus2_2021}]{ is a research project. However the authors chose to use their own implementation of the orchestration system instead of going for Kubernetes. This framework is not sufficient for our needs.
	}
	\item[\cite{smartfog_fogflow_2022}]{ is a mature framework coming from the research world. It is the result of a long list of papers \improvement{cite the list of fogflow papers?} that make it what it is today: a great open framework for early adopters of the Fog technology. Especially since the project is part of the Fiware \todo{cite fiware} backed by a number of great enterprises. However, for our needs, the framework is not appropriate. It is too complicated to understand fastly, making the modification part tricky. Especially since we would have to implement the auctioning mechanisms on top of it, and enable messaging on top of its existing stack. For these reasons, the framework is not interesting for us.}
\end{description}

Both of the options look like they are actively maintained, with an emphasize over \cite{smartfog_fogflow_2022} since the commits usually dates back days instead of months as in \cite{deng_fogbus2_2021}.
However, both options are not sufficient for us and we proceeded with developing our own solutions, fit to our needs. An additional benefit of this is the coupling of the framework with Grid'5000 as our testing and experimental ground.

\subsubsection{A framework to rule them all}

I coded this framework in Rust. The use of this modern language present several benefits, in no particular order:\improvement{Put an order}
\begin{itemize}
	...
	\item Rust is efficient without without being a low-level coding language such as C, making it ideal if the framework ever need to accomplish high efficiency tasks such as routing \improvement{Add a citation to the appropriate section}.
	\item 
\end{itemize}

\subsubsection{Benefits and limitations}

The creation of our own framework comes with benefits and a price.

The use of Rust, a strongly typed language, enables the generation of standard OPEN API v3 documentation for the routes create for the \gls{REST} \gls{API} This means the project can be shared with most of the endpoints documented in their intentions, their expected data type input as well as output. Additionally, since the framework is implemented under the clean architecture concept, this enables the change of the underlying technology if necessary. For example, one would be able to extend the framework to use another orchestrator instead of Kubernetes – granted this would still take a great amount of time, but the whole application would not require to be changed, in fact, only the part that enables the communication with the orchestrator. This also means the architecture enables the modification of the behavior of the framework regarding placement of functions, a necessary feature when it comes to compare against state-of-the-art research placement algorithms. \improvement{rework the sentence}

The current implementation is not complete yet compared to the initial ideas. Time is a constraint. The main limitation – artificial – is the tree topology we use as a simplification. In the future, we will extend the approach to a real graph topology to reflect the possibilities of the real world.

Another simplification is the absence of any security and payment mechanism. Indeed, we simplified this aspect, deemed not crucial for the purpose of research. For example, the step of payment is not implemented because it would essentially introduce a difficulty for no extra benefits to the task of function placement.

\subsubsection{Running the framework}

We planned from the beginning to run this framework on Grid'5000.

Grid'5000 is a large-scale test bed for experiments. It is a cluster of multiple data centers geographically scattered in research centers mainly in France. Each site offer from tens to hundreds of nodes to reserve for experimentation. Each node is configurable from its kernel to the software that runs on it. Other configurations include networking and size of the virtual instances, i.e. the resources reserved.

This way we can benefit from a controlled environment in which to run reproducible experiments thanks to a level of control not available in public Clouds. Another key point is scalability. We can expand our Fog network to any arbitrary number of nodes to experimentally validate our concepts. The addition of both benefits allows us to select the resources allowed per Fog nodes, and network conditions in between the nodes. Thus, we can perfectly model the Fog experimentally. For our use, we primarily focus on bandwidth and latency. Indeed, the constraints on the resources will come after having proven the framework is able to run on a network stack matching the Fog.

Using Grid'5000 can be achieved in a number of ways. The first—the legacy one—is through SSH and the execution of a number of bash scripts that create and control both the environment and the flow of the experiment.
Another way has been introduced thanks to the development of EnosLib \cite{cherrueau_enoslib_2022}. This library allows the complete use of Grid'5000 as the legacy, with one catch. One does not have to cope with the documentation of Grid'5000 and to learn its specific points and tricks to make things work. Everything is included in this Python library. Of course, documentation is still necessary to understand the various functionalities, but when lacking, one does have another way to solve his problem than to rely on bothering people through a mailing list: the source code is available and sufficient to understand the implementation in less time it takes to explain the problem to a third party.

Thanks to this library, a command line wrapper can be developed easily. The resulting script is simpler than using a bunch of bash scripts, especially since it is executed on the developer's hardware. Every SSH trick, copy, redirection and even standard installations are handled. For example, using docker is as simple as instantiating a Python context stating to run docker.
...
So this fastens development time by a lot. Another interesting feature that I will be using in the extremely near future is the data collection. This library has been thought to be running experiments, and as such, simplifies data collection. One interesting aspect is the use of Prometheus endpoints. \improvement{What is Prometheus. Intro needed}. Through the use of appropriate crates (libraries) in Rust, I would be able to produce endpoints on my framework that will be connected to EnosLib and log interesting metrics: latency between Fog nodes, load on the Fog node, number of instantiated functions, etc. And to connect those endpoints automatically to EnosLib, this way at the end of the experiment, I would be able to retrieve all those metrics in a simple way.


\subsubsection{Our evaluation plan}

We plan to evaluate the framework in order to publish a paper by the end of my internship \improvement{State the end of the internship in the introduction}. To do so, we divided our strategy into two levels, in order to at least be able to publish a statement to tell the community that a framework is in the works.
...
Economic efficiency aims to look at whether the Fog node earns enough “money” to be profitable for its owner. Profitability would be an incentive for the node to stay in the network. One experience could look at “what happens when unprofitable nodes leave the network because they went bankrupt?”


\subsection{Other work accomplished}
\iffalse
Developing the framework was not the first work to be started. First I had to be at ease with the literature of the fields this project interacts with. This means I wanted to be reassured about what already has been accomplished in the bibliographic report in terms of:
\begin{enumerate}
...
This meant a number of hours dedicated to find an answer to those questions and go further than the bibliographic report. \cref{enumerate:placement, enumerate:alreadytested, enumerate:frameworklimitations} lead to the initiative of a new framework fitting our particular needs and profiting from advantages such as Grid'5000—that kind of test bed is not usual in the available literature.

\cref{enumerate:fogapplications} led to find \improvement{Find BeFaaS citation}. This paper uses an openly available Fog application. Though it is still artificial—image recognition is basically pixel remapping on an image in order to “simulate” that. It is the closest I found to a Fog application. Of course a number of \gls{FaaS} applications are available. However one would have to go through all the different technologies to find code compatible with a platform. This entanglement is particularly visible with open private Cloud solutions—such as AWS'. Meaning reusing the code is not possible without both rewriting it and logically replacing its components. Finally, the main issue with this approach is the fact it is custom tailored for the Fog. What we want is to first test the Fog for Fog applications and only then test it with normal applications to finalize its integration to the current Cloud offerings.

Finally, we went with \improvement{cite BeFaas here} and actually rewrote their simple application with Rust in order to:
\begin{itemize}
	\item get rid of their library integration that they made for their testing;
	\item get rid of Terraform, as it would have been an annoying additional layer;
	\item get rid of JavaScript, as this would be unlikely to be a “high-performance” language used for applications running under constraints and likely paying for their resource consumptions.
\end{itemize}
In the end, the application was rewritten in Rust and that benefited the whole project as I also planned to use this language for the framework itself. Thus it was a good training and a good way to find the appropriate crates—modules—that I would use later in the framework (a good example is the crate I use for the \gls{REST} \gls{API} where I was hesitant to go with Rocket \improvement{cite Rocket?}, thus testing another option made me realize the benefits of using the other).
...
We also hope to participate in the Terra Forma \cite{longuevergne_terra_2022} CNRS-backed project to both help and experiment on a real Fog test bed. Indeed the project will monitor part of the French territory under a dense net of multiple \gls{IoT} sensors. We hope to demonstrate the power of the Fog reducing bandwidth consumption and enabling real-time decision taking (drone control, etc.). In such a project, we will be able to contribute our framework running in the real world, our placement mechanism as well an economically viable solution and, lastly, real Fog applications at scale—something scarce in the literature.
