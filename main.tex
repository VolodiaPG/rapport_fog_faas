...
\section{Committing to solving problems}

In \ref{sec:curated} we identified four main axis of improvements in the field. All are connected through one gap: comparison. Currently, comparison is not enabled in the field. How would one know the performance gap between MEC and Fog to steer the latter in the right direction. We are lacking the proper experimental framework. Simulations show metrics one could gain considering Fog. However it does not highlight real implementation challenges, limitations and needed improvements to use the technology. At last, simulation does not enable the study of real use cases. Currently there are none serious.

What this calls for is a framework. A piece of open software sufficiently maleable for anyone to measure the differences of the propositions against other's.
...
This introduces the idea of a new open framework that enables scientists to both develop real world usage for the Fog as well as test their ideas for placement and scheduling of such applications. Ideally this platforms begs to use \gls{FaaS} as its underlying paradigm for guest applications, or should I say functions.
...
\begin{enumerate}
	...
	\item The technology stack needs to enable cooperation
	\item Being a scientific piece of software, it should be expected to perform some shortcuts \footnote{For instance implement a routing mechanism instead of using a production one in the cluster. This both simplified development and would enable better monitoring and debug afterwards.}
\end{enumerate}

For those reasons I went with Rust as the development language. The novelty of the language and paradigm make the toolchain a breeze to use on any computer. Additionally the paradigm itself bans any developer errors from the project, especially when considering error handling mechanisms (Exceptions are not a necessary evil). To finish, Rust's compiler is the only one sufficienty advanced to actually help with errors, race conditions, etc. Plus it is performant enough so that the developed applications is not confined to a control role but can also take part in the data plane. Which is handy when it comes to developing a simple and understandable solution.

The project has been available on Github from the day-0. Being a REST-based suite of applications. All the documentation is compatible with the OpenAPI v3 standard. A must when it comes to sharing any applications. This makes the project compliant with the reproducible science mantra.

The framework is composed of two parts: the fog node part and the market part. The fog node is the part that communicates with both Kubernetes and OpenFaaS of the node (being the Cloud or any other node). The market part is the trusted one, taking the final placement decision and handling the economics of the auctions. For now, auctions are not involving any relations of money, as this platform is purely experimental.
...
Grid5000 is a grid of clusters situated at different locations all around France. The idea is to let scientitic use its nodes for demanding applications or experiments. It even has been engineering towards this particular end by making sure most if not all variations are monitored and mastered. EnoLib is a tool developed in my team, at Myriads that allows for a simpler control of the workflows sent to the nodes. Instead of having to deal with connections issues and bash scripts all around the place, it offers a Python API that does it for us. We can now concentrate on purely the experiment and its settings instead of having to deal with issues in Grid5000's documentation. Which is sweet, because it is easier to deal with missing documentation in a Python library instead of whatever is Grid5000.
