...
The report is divided as follows. \Cref{sec:background} further differentiates Fog and \gls{MEC}, discuss \gls{FaaS} and provides application examples. \Cref{sec:platforms} presents \gls{FaaS} platforms and explains our choice of Fog framework to develop our placement strategy on. Then \cref{sec:placement} discusses literature about function placement in the Fog, identifies challenges from the previously examined research and introduces an additional consideration of the Fog node ownership before exposing our placement strategy. \Cref{sec:implementingourstrat} describes our implementation. Then \cref{sec:evaluating} offers an overview of our planned evaluation process. \cref{sec:additionalwork} focus on other work done before finally, \cref{sec:evofurure} brings future work directions.
...
\subsection{Fog node ownership and auctions}
...
\subsection{Placing functions thanks to auctions}
...
\improvement{Add an example of SLA an response}

\section{Implementing our placement strategy}
\label{sec:implementingourstrat}

\change{Move to the literature review/platforms}
